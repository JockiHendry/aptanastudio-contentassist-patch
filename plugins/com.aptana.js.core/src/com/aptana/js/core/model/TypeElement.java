...
public class TypeElement extends BaseElement
{
	...
	/**
	 * Mengembalikan satu atau lebih  <code>PropertyElement</code> berdasarkan nama.
	 * 
	 * @param name adalah nama yang akan dicari.
	 * @return <code>List</code> yang berisi <code>PropertyElement</code>.
	 */
	...
	protected int getPropertyIndex(String name)
	{
		...
		if (name != null && name.length() > 0 && this._properties != null)
		{
			...
			for (int i = 0; i < this._properties.size(); i++)
			{
				...
				// Sebisa mungkin kembalikan property yang memiliki dokumentasi (deskripsi)!!
				if (name.equals(property.getName()))
				{
					result = i;
