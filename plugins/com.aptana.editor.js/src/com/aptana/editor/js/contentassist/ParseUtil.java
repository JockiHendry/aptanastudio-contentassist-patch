...
public class ParseUtil
{
		...
		if (getPropertyNode != null)
		{
			...
			if (localScope != null)
			{
				...
				for (String type : typeList)
				{
					...
					// I don't want to make difference between static or instance methods of jQuery
					/*
	...
	/**
	 * Melakukan filtering <code>PropertyElement</code> yang ada, berusaha mengembalikan versi terdokumentasi
	 * bila ada.  Bila versi terdokumentasi tidak ditemukan, maka <code>PropertyElement</code> akan dipakai apa adanya.
	 * 
	 * @param sumber adalah <code>Collection</code> yang akan disaring.
	 * @return <code>Collection</code> yang telah disaring.
	 */
	public static List<PropertyElement> filterDokumentasi(Collection<? extends PropertyElement> sumber) {

		ArrayList<PropertyElement> listReturn = new ArrayList<PropertyElement>();
		
		for (PropertyElement propertyElement: sumber) {
			if (propertyElement.getDescription()==null || propertyElement.getDescription().length()==0) {
				// cari apakah ada versi yang terdokumentasi
				boolean adaVersiTerdokumentasi = false;
				for (PropertyElement p: sumber) {
					if (p.getDescription()!=null && p.getDescription().length() > 0) {
						adaVersiTerdokumentasi = true;
						break;
					}
				}
				if (!adaVersiTerdokumentasi) {
					listReturn.add(propertyElement);
				}
			} else {
				listReturn.add(propertyElement);
			}
